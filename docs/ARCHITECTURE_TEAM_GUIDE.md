...
9. **علامة `+` في تاريخ ISO داخل رابط PostgREST** تُقرأ كمسافة فتفشل الاستعلام
   (`invalid input syntax for type timestamp`). لفّ التاريخ بـ
   `encodeURIComponent(...)` أو استخدم `.toUTC().toISO()` (ينتهي بـ `Z` بلا `+`).
10. **اختلاف مخطط الخدمات بين المصادر**: شيتات جوجل تستخدم عمود `deleted`
    (حذف ناعم)، بينما جداول Supabase تستخدم `active`. أي استعلام خدمات لازم
    يتفرّع على `db_type` — لا تطلب عموداً غير موجود في المصدر الآخر وإلا يفشل
    الاستعلام صامتاً مع `onError: continueRegularOutput` وترجع النتيجة فاضية.
11. **الكتابة على مصدر العيادة تحتاج مرآة موحّدة**: أي أداة تعدّل موعداً في
    قاعدة العيادة (حجز/إلغاء) يجب أن تعكس التغيير في `unified_appointments`
    (RPC `sync_import_appointment` أو PATCH مباشر على `external_id`) — وإلا
    يظل الداشبورد يعرض الحالة القديمة حتى المزامنة التالية.

> **ملاحظة اختبار (2026-07-21):** مسار عيادات Supabase اختُبر end-to-end على
> قاعدة بيانات حقيقية (حجز → استعلام → منع تعارض → إلغاء → جلب الخدمات + مرآة
> موحّدة). الأخطاء 9/10/11 أعلاه اكتُشفت وأُصلحت خلال هذا الاختبار — كانت كامنة
> لأن المسار كان مبنياً لكن غير مُختبَر بعيادة Supabase فعلية.

12. **حساب الأوقات الشاغرة كان يخلط التوقيت حسب المصدر** (اكتُشف بجولة QA
    2026-07-21): "Get Available Slots" كان يعامل وقت الموعد كـ UTC حرفياً
    (`slice(0,19)+'Z'`). عيادات الشيت تخزّن wall-clock مع `+03:00` فتصادف
    تشتغل، لكن عيادات Supabase تخزّن UTC حقيقي (10:00 الرياض = 07:00Z) فينتج
    فرق 3 ساعات → **البوت يعرض أوقاتاً محجوزة كأنها شاغرة**. الإصلاح: تحويل كل
    وقت لتوقيت الرياض عبر Luxon (`DateTime … setZone('Asia/Riyadh')`) ومقارنة
    دقائق-اليوم — يعمل للمصدرين. (منطق منع التعارض في Book كان سليماً أصلاً
    لأنه يستخدم `new Date().getTime()` = epoch حقيقي.)
13. **أسطر جديدة فعلية داخل تعبير n8n `={{ '...' }}`** تكسر المُحلّل
    (`invalid syntax`) — عقدة "Reply Linked" فشلت لكل مريض يشارك جهة اتصاله.
    استخدم `\n` (شرطة+n) داخل السلسلة، لا سطراً فعلياً. القاعدة العامة: أي
    نص عربي متعدد الأسطر في Set/httpRequest expression لازم `\n` لا newline خام.
